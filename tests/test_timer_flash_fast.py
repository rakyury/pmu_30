...
def build_test_config() -> bytes:
    ...
    1. Timer FLASH (ch 200): mode=BLINK, trigger=DIN0, on=200ms, off=200ms
    ...
    When button pressed (1):  Timer runs, LED blinks 0.2s ON / 0.2s OFF
    """
    channels = []
    ...
    timer_config = build_timer_config(
        ...
        on_time_ms=200,        # 0.2 second ON
        off_time_ms=200        # 0.2 second OFF
    )
...
class TimerFlashTest:
    ...
    def upload_config(self, config: bytes) -> bool:
        ...
        print(f"    Timer: BLINK mode, trigger=DIN0, 0.2s ON / 0.2s OFF")
    ...
    def monitor_flash(self, duration: float = 15.0):
        ...
        Expected:
        - Button released (0): LED OFF (immediately)
        - Button pressed (1):  LED blinks (0.2s ON, 0.2s OFF)
        """
        ...
        print("    - Button PRESSED:  LED blinks (0.2s ON / 0.2s OFF)")
        print()
    ...
    def run(self) -> int:
        print("=" * 60)
        print("PMU-30 Timer FLASH Fast (Triggered Blink) Test")
        print("=" * 60)
        print(f"Port: {self.port_name}")
        print("\nTest: DIN0 (trigger) -> Timer FLASH -> Power Output -> LED")
        print("Timer: BLINK mode, 0.2 second ON, 0.2 second OFF")
        print("\nExpected:")
