#!/usr/bin/env python3
"""
PMU-30 Logic GT (Greater Than) Test

Tests channel linking with Logic GT comparison:
  DIN0 (button, ch 50) -> Logic GT (ch 200) -> Power Output (ch 100) -> LED

Logic GT configuration:
  - Input: DIN0 (button value: 0 or 1)
  - Compare value: 0
  - Operation: input > 0

Expected behavior:
  - Button released (0) -> 0 > 0 = FALSE -> Output OFF -> LED OFF
  - Button pressed  (1) -> 1 > 0 = TRUE  -> Output ON  -> LED ON

Same behavior as direct linking, but using GT comparison function.

Usage:
    python tests/test_logic_gt.py [COM_PORT]
"""
...
LOGIC_OP_GT = 0x10  # Greater Than: input > compare_value

# Channel IDs
CH_DIN0 = 50        # Digital input 0 (button)
CH_LOGIC_GT = 200   # Logic GT channel
...
def build_test_config() -> bytes:
    """
    Build binary config for Logic GT test.

    Config contains 2 channels:
    1. Logic GT (ch 200): input=DIN0(50), compare_value=0, result = input > 0
    2. Power Output (ch 100): source=Logic GT (200), hw_index=1 (LED)

    Chain: DIN0 (50) -> Logic GT (200) -> Power Output (100) -> LED

    When button pressed:  1 > 0 = TRUE  -> LED ON
    When button released: 0 > 0 = FALSE -> LED OFF
    """
    channels = []

    # 1. Logic GT channel (ch 200)
    #    Input: DIN0 (ch 50)
    #    Compare: input > 0
    logic_config = build_logic_config(
        operation=LOGIC_OP_GT,
        inputs=[CH_DIN0],
        compare_value=0,  # Compare with 0
        invert=False
    )
    logic_header = build_channel_header(
        channel_id=CH_LOGIC_GT,
...
class LogicGtTest:
    """Test Logic GT channel functionality."""
    ...
    def upload_config(self, config: bytes) -> bool:
        print("\n[2] Uploading Logic GT config...")
        print(f"    Config size: {len(config)} bytes")
        print(f"    Logic: input > 0")
    ...
    def monitor_logic_gt(self, duration: float = 15.0):
        """
        Monitor Logic GT: input > 0

        Expected:
        - Button released (0) -> 0 > 0 = FALSE -> LED OFF
        - Button pressed  (1) -> 1 > 0 = TRUE  -> LED ON
        """
        print("\n[3] Monitoring Logic GT chain...")
        print(f"    Duration: {duration}s")
        print("    Chain: DIN0 (button) -> Logic GT (>0) -> Power Output -> LED")
        print()
        print("    Logic: button_value > 0")
        print("    - Button RELEASED (0 > 0 = FALSE) -> LED OFF")
        print("    - Button PRESSED  (1 > 0 = TRUE)  -> LED ON")
        print()
        ...
        try:
            while time.time() - start < duration:
                ...
                if outputs is not None and dins is not None:
                    ...
                    # Logic: button > 0
                    # button=1 -> 1>0=TRUE  -> LED ON
                    # button=0 -> 0>0=FALSE -> LED OFF
                    expected_output = button_pressed  # Same as direct link
                    ...
                    if last_button is not None and button_pressed != last_button:
                        ...
                        logic_result = "1>0=TRUE" if button_pressed else "0>0=FALSE"
        ...
        if match_rate >= 95:
            print("    [OK] Logic GT (input > 0) working correctly!")
            return True
        print("    [FAIL] Logic GT not working as expected")
        return False

    def run(self) -> int:
        print("=" * 60)
        print("PMU-30 Logic GT (Greater Than) Test")
        print("=" * 60)
        print(f"Port: {self.port_name}")
        print("\nTest: DIN0 -> Logic GT (>0) -> Power Output -> LED")
        print("Logic: button_value > 0")
        ...
        try:
            ...
            if not self.monitor_logic_gt(duration=12.0):
                return 1

            print("\n" + "=" * 60)
            print("ALL TESTS PASSED - Logic GT (input > 0) working!")
            print("=" * 60)
            return 0
