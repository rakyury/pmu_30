#!/usr/bin/env python3
"""
PMU-30 Logic XOR Test

Tests Logic XOR function with two inputs:
  - DIN0 (button, ch 50)
  - Number "one" (constant 1, ch 201)

Chain:
  Number "one" (ch 201) ─┬─> Logic XOR (ch 200) -> Power Output (ch 100) -> LED
  DIN0 (button, ch 50) ──┘

Logic XOR configuration:
  - Inputs: [DIN0, Number "one"]
  - Operation: input0 XOR input1 = DIN0 XOR 1

Expected behavior:
  - Button released (0) -> 0 XOR 1 = 1 -> LED ON
  - Button pressed  (1) -> 1 XOR 1 = 0 -> LED OFF

Usage:
    python tests/test_logic_xor.py [COM_PORT]
"""
...
def build_number_config(value: int) -> bytes:
    return struct.pack('<iiii??2s', value, 0, value, 1, True, False, b'\x00\x00')
...
def build_test_config() -> bytes:
    ...
    # Number "one" (ch 201)
    number_config = build_number_config(value=1)
    number_header = build_channel_header(CH_NUMBER_ONE, CH_TYPE_NUMBER, name="one",
                                          config_size=len(number_config))
    channels.append(number_header + number_config)

    # Logic XOR (ch 200)
    logic_config = build_logic_config(LOGIC_OP_XOR, [CH_DIN0, CH_NUMBER_ONE])
    logic_header = build_channel_header(CH_LOGIC_XOR, CH_TYPE_LOGIC, name="XOR",
    ...
    output_header = build_channel_header(CH_OUTPUT_0, CH_TYPE_POWER_OUTPUT, hw_device=HW_DEVICE_PROFET,
                                          hw_index=1, source_id=CH_LOGIC_XOR, name="LED",
...
class LogicXorTest:
    ...
    def monitor(self, duration: float = 12.0):
        print("\n[3] Monitoring Logic XOR chain...")
        print("    Logic: DIN0 XOR 1")
        print("    - Button RELEASED (0 XOR 1 = 1) -> LED ON")
        print("    - Button PRESSED  (1 XOR 1 = 0) -> LED OFF")
        print()
            ...
            while time.time() - start < duration:
                ...
                if outputs is not None and dins is not None:
                    ...
                    if last_button is not None and button_pressed != last_button:
                        ...
                        print(f"    Button {state}, LED {out_state}")
        ...
        if match_rate >= 95:
            print("    [OK] Logic XOR working!")
            return True
        print("    [FAIL] Logic XOR not working")
        return False

    def run(self) -> int:
        print("=" * 60)
        print("PMU-30 Logic XOR Test")
        print("=" * 60)
