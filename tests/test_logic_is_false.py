#!/usr/bin/env python3
"""
PMU-30 Logic IS_FALSE Test

Tests channel linking with Logic IS_FALSE function:
  DIN0 (button, ch 50) -> Logic IS_FALSE (ch 200) -> Power Output (ch 100) -> LED

IS_FALSE returns 1 when input equals 0 (same behavior as NOT).

Expected behavior:
  - Button released (0) -> IS_FALSE(0) = 1 -> Output ON  -> LED ON
  - Button pressed  (1) -> IS_FALSE(1) = 0 -> Output OFF -> LED OFF

Usage:
    python tests/test_logic_is_false.py [COM_PORT]
"""
...
LOGIC_OP_IS_FALSE = 0x07  # Returns 1 if input == 0

# Channel IDs
CH_DIN0 = 50             # Digital input 0 (button)
CH_LOGIC_IS_FALSE = 200  # Logic IS_FALSE channel
...
def build_test_config() -> bytes:
    """
    Build binary config for Logic IS_FALSE test.

    Config contains 2 channels:
    1. Logic IS_FALSE (ch 200): input=DIN0(50), output=1 if input==0
    2. Power Output (ch 100): source=Logic IS_FALSE (200), hw_index=1 (LED)

    Chain: DIN0 (50) -> Logic IS_FALSE (200) -> Power Output (100) -> LED
    """
    channels = []

    # 1. Logic IS_FALSE channel (ch 200)
    logic_config = build_logic_config(
        operation=LOGIC_OP_IS_FALSE,
        inputs=[CH_DIN0],
        compare_value=0,
        invert=False
    )
    logic_header = build_channel_header(
        channel_id=CH_LOGIC_IS_FALSE,
...
class LogicIsFalseTest:
    """Test Logic IS_FALSE channel functionality."""
    ...
    def upload_config(self, config: bytes) -> bool:
        print("\n[2] Uploading Logic IS_FALSE config...")
    ...
    def monitor_is_false(self, duration: float = 15.0):
        """
        Monitor Logic IS_FALSE: output = 1 when input == 0.

        Expected:
        - Button released (0) -> IS_FALSE(0) = 1 -> LED ON
        - Button pressed  (1) -> IS_FALSE(1) = 0 -> LED OFF
        """
        print("\n[3] Monitoring Logic IS_FALSE chain...")
        print(f"    Duration: {duration}s")
        print("    Chain: DIN0 (button) -> Logic IS_FALSE -> Power Output -> LED")
        print()
        print("    Expected behavior:")
        print("    - Button RELEASED (0) -> IS_FALSE = 1 -> LED ON")
        print("    - Button PRESSED  (1) -> IS_FALSE = 0 -> LED OFF")
        print()
        ...
        try:
            while time.time() - start < duration:
                ...
                if outputs is not None and dins is not None:
                    ...
                    # IS_FALSE: returns 1 when input == 0
                    expected_output = not button_pressed

                    if output_on == expected_output:
                        matches += 1

                    # Check virtual channel value in telemetry
                    if channels and CH_LOGIC_IS_FALSE in channels:
                        channel_found += 1
                        logic_value = channels[CH_LOGIC_IS_FALSE]
                        expected_logic = 0 if button_pressed else 1
                    ...
                    if last_button is not None and button_pressed != last_button:
                        ...
                        logic_val = channels.get(CH_LOGIC_IS_FALSE, "N/A") if channels else "N/A"
        ...
        if match_rate >= 95 and channel_rate >= 90:
            print("    [OK] Logic IS_FALSE chain working correctly!")
            ...
            print("    [FAIL] Logic IS_FALSE chain not working as expected")
            return False

    def run(self) -> int:
        print("=" * 60)
        print("PMU-30 Logic IS_FALSE Test")
        print("=" * 60)
        print(f"Port: {self.port_name}")
        print("\nTest: DIN0 -> Logic IS_FALSE -> Power Output -> LED")
        print("IS_FALSE returns 1 when input == 0")
        ...
        try:
            ...
            if not self.monitor_is_false(duration=15.0):
                return 1

            print("\n" + "=" * 60)
            print("ALL TESTS PASSED - Logic IS_FALSE working!")
            print("=" * 60)
            return 0
