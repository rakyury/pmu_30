#!/usr/bin/env python3
"""
PMU-30 Logic LT (Less Than) Test

Tests channel linking with Logic LT comparison:
  DIN0 (button, ch 50) -> Logic LT (ch 200) -> Power Output (ch 100) -> LED

Logic LT configuration:
  - Input: DIN0 (button value: 0 or 1)
  - Compare value: 1
  - Operation: input < 1

Expected behavior:
  - Button released (0) -> 0 < 1 = TRUE  -> Output ON  -> LED ON
  - Button pressed  (1) -> 1 < 1 = FALSE -> Output OFF -> LED OFF

Opposite behavior from GT test (LED on when released, off when pressed).

Usage:
    python tests/test_logic_lt.py [COM_PORT]
"""
...
LOGIC_OP_LT = 0x12  # Less Than: input < compare_value

# Channel IDs
CH_DIN0 = 50        # Digital input 0 (button)
CH_LOGIC_LT = 200   # Logic LT channel
...
def build_test_config() -> bytes:
    """
    Build binary config for Logic LT test.

    Config contains 2 channels:
    1. Logic LT (ch 200): input=DIN0(50), compare_value=1, result = input < 1
    2. Power Output (ch 100): source=Logic LT (200), hw_index=1 (LED)

    Chain: DIN0 (50) -> Logic LT (200) -> Power Output (100) -> LED

    When button pressed:  1 < 1 = FALSE -> LED OFF
    When button released: 0 < 1 = TRUE  -> LED ON
    """
    channels = []

    # 1. Logic LT channel (ch 200)
    #    Input: DIN0 (ch 50)
    #    Compare: input < 1
    logic_config = build_logic_config(
        operation=LOGIC_OP_LT,
        inputs=[CH_DIN0],
        compare_value=1,  # Compare with 1
        invert=False
    )
    logic_header = build_channel_header(
        channel_id=CH_LOGIC_LT,
...
class LogicLtTest:
    """Test Logic LT channel functionality."""
    ...
    def upload_config(self, config: bytes) -> bool:
        print("\n[2] Uploading Logic LT config...")
        print(f"    Config size: {len(config)} bytes")
        print(f"    Logic: input < 1")
    ...
    def monitor_logic_lt(self, duration: float = 15.0):
        """
        Monitor Logic LT: input < 1

        Expected:
        - Button released (0) -> 0 < 1 = TRUE  -> LED ON
        - Button pressed  (1) -> 1 < 1 = FALSE -> LED OFF
        """
        print("\n[3] Monitoring Logic LT chain...")
        print(f"    Duration: {duration}s")
        print("    Chain: DIN0 (button) -> Logic LT (<1) -> Power Output -> LED")
        print()
        print("    Logic: button_value < 1")
        print("    - Button RELEASED (0 < 1 = TRUE)  -> LED ON")
        print("    - Button PRESSED  (1 < 1 = FALSE) -> LED OFF")
        print()
        ...
        try:
            while time.time() - start < duration:
                ...
                if outputs is not None and dins is not None:
                    ...
                    # Logic: button < 1
                    # button=0 -> 0<1=TRUE  -> LED ON
                    # button=1 -> 1<1=FALSE -> LED OFF
                    expected_output = not button_pressed  # INVERTED from GT!
                    ...
                    if last_button is not None and button_pressed != last_button:
                        ...
                        logic_result = "1<1=FALSE" if button_pressed else "0<1=TRUE"
        ...
        if match_rate >= 95:
            print("    [OK] Logic LT (input < 1) working correctly!")
            return True
        print("    [FAIL] Logic LT not working as expected")
        return False

    def run(self) -> int:
        print("=" * 60)
        print("PMU-30 Logic LT (Less Than) Test")
        print("=" * 60)
        print(f"Port: {self.port_name}")
        print("\nTest: DIN0 -> Logic LT (<1) -> Power Output -> LED")
        print("Logic: button_value < 1")
        print("\nExpected: LED ON when released, LED OFF when pressed")
        ...
        try:
            ...
            if not self.monitor_logic_lt(duration=12.0):
                return 1

            print("\n" + "=" * 60)
            print("ALL TESTS PASSED - Logic LT (input < 1) working!")
            print("=" * 60)
            return 0
