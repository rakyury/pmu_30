#!/usr/bin/env python3
"""
PMU-30 Logic NEQ (Not Equal) Test

Tests Logic NEQ function:
  - DIN0 (button 0, ch 50) != threshold

Chain:
  DIN0 (button, ch 50) -> Logic NEQ (ch 200, threshold=0) -> Power Output (ch 100) -> LED

Logic NEQ configuration:
...
Usage:
    python tests/test_logic_neq.py [COM_PORT]
"""
...
def build_test_config() -> bytes:
    ...
    # Logic NEQ (ch 200) - threshold = 0
    logic_config = build_logic_config(LOGIC_OP_NEQ, [CH_DIN0], threshold=0)
    logic_header = build_channel_header(CH_LOGIC_NEQ, CH_TYPE_LOGIC, name="NEQ",
    ...
    output_header = build_channel_header(CH_OUTPUT_0, CH_TYPE_POWER_OUTPUT, hw_device=HW_DEVICE_PROFET,
                                          hw_index=1, source_id=CH_LOGIC_NEQ, name="LED",
...
class LogicNeqTest:
    ...
    def upload_config(self, config: bytes) -> bool:
        print("\n[2] Uploading Logic NEQ config (threshold=0)...")
    ...
    def monitor(self, duration: float = 12.0):
        print("\n[3] Monitoring Logic NEQ chain...")
        print("    Logic: DIN0 != 0")
        print("    - Button RELEASED (0) -> 0 != 0 = FALSE -> LED OFF")
        print("    - Button PRESSED  (1) -> 1 != 0 = TRUE  -> LED ON")
        print()
        ...
        if match_rate >= 95:
            print("    [OK] Logic NEQ working!")
            return True
        print("    [FAIL] Logic NEQ not working")
        return False

    def run(self) -> int:
        print("=" * 60)
        print("PMU-30 Logic NEQ (!=) Test")
        print("=" * 60)
        print(f"Port: {self.port_name}")
        print("\nExpected: LED ON when button pressed (1 != 0)")
