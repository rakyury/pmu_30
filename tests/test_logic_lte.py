#!/usr/bin/env python3
"""
PMU-30 Logic LTE (Less Than or Equal) Test

Tests Logic LTE function:
  - DIN0 (button 0, ch 50) <= threshold

Chain:
  DIN0 (button, ch 50) -> Logic LTE (ch 200, threshold=0) -> Power Output (ch 100) -> LED

Logic LTE configuration:
  - Input: DIN0 (value 0 or 1)
  - Threshold: 0
  - Operation: input <= threshold

Expected behavior:
  - Button released (0) -> 0 <= 0 = TRUE  -> LED ON
  - Button pressed  (1) -> 1 <= 0 = FALSE -> LED OFF

Usage:
    python tests/test_logic_lte.py [COM_PORT]
"""
...
def build_test_config() -> bytes:
    ...
    # Logic LTE (ch 200) - threshold = 0
    logic_config = build_logic_config(LOGIC_OP_LTE, [CH_DIN0], threshold=0)
    logic_header = build_channel_header(CH_LOGIC_LTE, CH_TYPE_LOGIC, name="LTE",
    ...
    output_header = build_channel_header(CH_OUTPUT_0, CH_TYPE_POWER_OUTPUT, hw_device=HW_DEVICE_PROFET,
                                          hw_index=1, source_id=CH_LOGIC_LTE, name="LED",
...
class LogicLteTest:
    ...
    def upload_config(self, config: bytes) -> bool:
        print("\n[2] Uploading Logic LTE config (threshold=0)...")
    ...
    def monitor(self, duration: float = 12.0):
        print("\n[3] Monitoring Logic LTE chain...")
        print("    Logic: DIN0 <= 0")
        print("    - Button RELEASED (0) -> 0 <= 0 = TRUE  -> LED ON")
        print("    - Button PRESSED  (1) -> 1 <= 0 = FALSE -> LED OFF")
        print()
        ...
        if match_rate >= 95:
            print("    [OK] Logic LTE working!")
            return True
        print("    [FAIL] Logic LTE not working")
        return False

    def run(self) -> int:
        print("=" * 60)
        print("PMU-30 Logic LTE (<=) Test")
        print("=" * 60)
        print(f"Port: {self.port_name}")
        print("\nExpected: LED ON when button released (0 <= 0)")
