#!/usr/bin/env python3
"""
PMU-30 Logic GTE (Greater Than or Equal) Test

Tests Logic GTE function:
  - DIN0 (button 0, ch 50) >= threshold

Chain:
  DIN0 (button, ch 50) -> Logic GTE (ch 200, threshold=1) -> Power Output (ch 100) -> LED

Logic GTE configuration:
...
Usage:
    python tests/test_logic_gte.py [COM_PORT]
"""
...
def build_test_config() -> bytes:
    ...
    # Logic GTE (ch 200) - threshold = 1
    logic_config = build_logic_config(LOGIC_OP_GTE, [CH_DIN0], threshold=1)
    logic_header = build_channel_header(CH_LOGIC_GTE, CH_TYPE_LOGIC, name="GTE",
    ...
    output_header = build_channel_header(CH_OUTPUT_0, CH_TYPE_POWER_OUTPUT, hw_device=HW_DEVICE_PROFET,
                                          hw_index=1, source_id=CH_LOGIC_GTE, name="LED",
...
class LogicGteTest:
    ...
    def upload_config(self, config: bytes) -> bool:
        print("\n[2] Uploading Logic GTE config (threshold=1)...")
    ...
    def monitor(self, duration: float = 12.0):
        ...
        if match_rate >= 95:
            print("    [OK] Logic GTE working!")
            return True
        print("    [FAIL] Logic GTE not working")
        return False

    def run(self) -> int:
        print("=" * 60)
        print("PMU-30 Logic GTE (>=) Test")
        print("=" * 60)
