#!/usr/bin/env python3
"""
PMU-30 Logic NOR Test

Tests Logic NOR function with two inputs:
  - DIN0 (button 0, ch 50)
  - DIN1 (button 1, ch 51)

Chain:
  DIN0 (button, ch 50) ─┬─> Logic NOR (ch 200) -> Power Output (ch 100) -> LED
  DIN1 (button, ch 51) ─┘

Logic NOR configuration:
  - Inputs: [DIN0, DIN1]
  - Operation: NOT(input0 OR input1) = NOT(DIN0 OR DIN1)

Expected behavior (NOR = NOT OR):
  - Both released (0,0) -> NOT(0 OR 0) = NOT(0) = 1 -> LED ON
  - DIN0 pressed  (1,0) -> NOT(1 OR 0) = NOT(1) = 0 -> LED OFF
  - DIN1 pressed  (0,1) -> NOT(0 OR 1) = NOT(1) = 0 -> LED OFF
  - Both pressed  (1,1) -> NOT(1 OR 1) = NOT(1) = 0 -> LED OFF

Usage:
    python tests/test_logic_nor.py [COM_PORT]
"""
...
def build_test_config() -> bytes:
    ...
    # Logic NOR (ch 200)
    logic_config = build_logic_config(LOGIC_OP_NOR, [CH_DIN0, CH_DIN1])
    logic_header = build_channel_header(CH_LOGIC_NOR, CH_TYPE_LOGIC, name="NOR",
    ...
    output_header = build_channel_header(CH_OUTPUT_0, CH_TYPE_POWER_OUTPUT, hw_device=HW_DEVICE_PROFET,
                                          hw_index=1, source_id=CH_LOGIC_NOR, name="LED",
...
class LogicNorTest:
    ...
    def monitor(self, duration: float = 12.0):
        print("\n[3] Monitoring Logic NOR chain...")
        print("    Logic: NOT(DIN0 OR DIN1)")
        print("    - Both RELEASED        -> LED ON")
        print("    - Any button PRESSED   -> LED OFF")
        print()
        ...
        try:
            while time.time() - start < duration:
                ...
                if outputs is not None and dins is not None:
                    ...
                    # NOR: NOT(A OR B)
                    expected_output = not (btn0 or btn1)
        ...
        if match_rate >= 95:
            print("    [OK] Logic NOR working!")
            return True
        print("    [FAIL] Logic NOR not working")
        return False

    def run(self) -> int:
        print("=" * 60)
        print("PMU-30 Logic NOR Test")
        print("=" * 60)
        print(f"Port: {self.port_name}")
        print("\nExpected: LED ON only when BOTH buttons released")
