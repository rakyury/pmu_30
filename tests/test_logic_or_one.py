#!/usr/bin/env python3
"""
PMU-30 Logic OR (always ON) Test
...
Chain:
  Number "one" (ch 201) ─┬─> Logic OR (ch 200) -> Power Output (ch 100) -> LED
  DIN0 (button, ch 50) ──┘

Logic OR configuration:
  - Inputs: [DIN0 (50), Number "one" (201)]
  - Operation: input0 OR input1 = DIN0 OR 1 = 1 (always ON)

Expected behavior:
  - Button released (0) -> 0 OR 1 = 1 -> LED ON
  - Button pressed  (1) -> 1 OR 1 = 1 -> LED ON

Same as direct linking, but demonstrates OR with multiple inputs.

Usage:
    python tests/test_logic_or_one_one.py [COM_PORT]
"""
...
def build_test_config() -> bytes:
    ...
    When button pressed:  1 OR 1 = 1 -> LED ON
    When button released: 0 OR 1 = 1 -> LED ON
    """
    channels = []
...
class LogicOrOneTest:
    ...
    def upload_config(self, config: bytes) -> bool:
        ...
        print(f"    Channels: Number(one=0), Logic OR, Power Output")
        print(f"    Logic: DIN0 AND one = DIN0 OR 0")
    ...
    def monitor_logic_or_one(self, duration: float = 15.0):
        """
        ...
        print("    Chain: DIN0 + Number(1) -> Logic OR -> Power Output -> LED")
        print()
        print("    Logic: DIN0 OR 1 = 1 (always ON)")
        print("    - Button RELEASED (0 OR 1 = 1) -> LED ON")
        print("    - Button PRESSED  (1 OR 1 = 1) -> LED ON")
        print()
        ...
        try:
            while time.time() - start < duration:
                ...
                if outputs is not None and dins is not None:
                    ...
                    # Logic: DIN0 OR 1 = 1 (always ON)
                    # button=1 -> 1 OR 1 = 1 -> LED ON
                    # button=0 -> 0 OR 1 = 1 -> LED ON
                    expected_output = True  # Always ON with OR 1
    ...
    def run(self) -> int:
        print("=" * 60)
        print("PMU-30 Logic OR (always ON) Test")
        print("=" * 60)
        print(f"Port: {self.port_name}")
        print("\nTest: DIN0 + Number(1) -> Logic OR -> Power Output -> LED")
        print("Logic: DIN0 OR 1 = 1 (always ON)")
        print("\nExpected: LED always ON (0 OR 1 = 1, 1 OR 1 = 1)")
        ...
        try:
            ...
            if not self.monitor_logic_or_one(duration=12.0):
                return 1
