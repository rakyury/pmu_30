...
**Модули (31 файл в firmware/src/)**:
| Модуль | Тесты | Покрытие | Приоритет |
|--------|-------|----------|-----------|
| pmu_protection.c | test_protection.c | ~85% | ✅ |
| pmu_can.c | test_can.c | ~80% | ✅ |
| pmu_logging.c | test_logging.c | ~85% | ✅ |
| pmu_ui.c | test_ui.c | ~80% | ✅ |
| pmu_lua.c | test_lua.c | ~90% | ✅ |
| pmu_channel.c | ❌ Нет | 0% | 🔴 Критичный |
| pmu_config_json.c | ❌ Нет | 0% | 🔴 Критичный |
| pmu_logic.c | ❌ Нет | 0% | 🔴 Критичный |
| pmu_profet.c | ❌ Нет | 0% | 🔴 Критичный |
| pmu_adc.c | ❌ Нет | 0% | 🟡 Важный |
| pmu_timer.c | ❌ Нет | 0% | 🟡 Важный |
| pmu_pid.c | ❌ Нет | 0% | 🟡 Важный |
| pmu_hbridge.c | ❌ Нет | 0% | 🟡 Важный |
| pmu_can_stream.c | ❌ Нет | 0% | 🟡 Важный |
| pmu_protocol.c | ❌ Нет | 0% | 🟡 Важный |
...
**Текущее покрытие**: 5/21 основных модулей (~24%)
...
```
tests/
├── ui/                    # PyQt6 диалоги (4 файла)
├── integration/           # Интеграция с эмулятором (13 файлов)
├── unit/                  # Unit тесты (1 файл)
├── test_config_*.py       # Валидация конфигов
├── test_protocol.py       # Протокол связи
├── test_telemetry.py      # Телеметрия
└── test_comm_manager.py   # Менеджер соединений
```

**Модули без тестов**:
| Категория | Модули | Тесты |
|-----------|--------|-------|
| Dialogs | 25+ диалогов | ~50% |
| Widgets | 15+ виджетов | ~20% |
| Tabs | 8 вкладок | 0% |
| Models | channel.py, undo_manager.py | ~30% |
| Controllers | device_controller.py | ~20% |
| Utils | theme.py, dbc_parser.py | ~10% |
...
### Неделя 1-2: Критичные тесты Firmware
1. ✅ test_channel.c
2. ✅ test_config_json.c
3. ✅ test_logic.c
4. ✅ test_profet.c

### Неделя 3: Важные тесты Firmware
5. test_adc.c
6. test_timer.c
7. test_pid.c
8. test_hbridge.c

### Неделя 4: Рефакторинг Configurator
9. Унификация диалогов
10. Расширение UI тестов

### Неделя 5: CI/CD и покрытие
11. GitHub Actions интеграция
12. Coverage reports
13. Pre-commit hooks
